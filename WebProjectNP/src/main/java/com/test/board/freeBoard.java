...
public class freeBoard extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		ArrayList<boardDTO> freeList = dao.freeList(map);

		for (boardDTO dto : freeList) {

			// 제목 자르기
			String title = dto.getFreeBoardTitle();
			if (title.length() > 20) {
		...
		if (n == 1) {
			...
			pagebar += String.format("<a href='/np/board/freeBoard.do?page=%d'><li>&lt;</li></a>", n - 1);
		}
		...
		while (!(loop > blockSize || n > totalPage)) {
			...
			if (nowPage == n) {
				pagebar += String.format(
						"<a href=\"/np/board/freeBoard.do?page=%d\" style='background-color: #92AC60;'><li style='left: 20px;\r\n"
								+ "	color: #fff;'>%d</li></a>",
						n, n);
			} else {
				pagebar += String.format(" <a href='/np/board/freeBoard.do?page=%d'><li>%d</li></a> ", n, n);
			}
		...
		if (n > totalPage) {
			...
			pagebar += String.format("<a href='/np/board/freeBoard.do?page=%d'><li>&gt;</li></a>", n);
		}
