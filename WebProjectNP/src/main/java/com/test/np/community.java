...
import com.test.board.allBoardDAO;
import com.test.board.allBoardDTO;
...
public class community extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 디비에서 데이터 가져오기	
		// all
		allBoardDAO dao3 = new allBoardDAO();
		ArrayList<allBoardDTO> allList = dao3.allList(map);
		for (allBoardDTO dto : allList) {
			// 제목 자르기
		...
		if (n == 1) {
			...
			pagebar += String.format("<a href='/np/community.do?page=%d'><li>&lt;</li></a>", n - 1);
		}
		...
		while (!(loop > blockSize || n > totalPage)) {
			...
			if (nowPage == n) {
				pagebar += String.format(
						"<a href=\"/np/community.do?page=%d\" style='background-color: #92AC60;'><li style='left: 20px;\r\n"
						+ "	color: #fff;'>%d</li></a>",
						n, n);
			} else {
				pagebar += String.format(" <a href='/np/community.do?page=%d'><li>%d</li></a> ", n, n);
			}
		...
		if (n > totalPage) { 
			...
			pagebar += String.format("<a href='/np/community.do?page=%d'><li>&gt;</li></a>", n);
		}
